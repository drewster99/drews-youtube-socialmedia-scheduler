...
7. **Cut promos from the source** *(optional)* — Promo Videos screen → "Generate from source" → pick which kinds (Hooks / Shorts / Segments) and whether to crop to 9:16. Claude proposes ranges from the parent's transcript; vertical-crop kinds are reframed to 9:16 on-device at cut time by head-tracking. Review the cards, dismiss any you don't want, then "Cut & insert selected" — the cut MP4s drop into the promo grid and run through the normal upload chain (AI title kept from the proposal, AI description / tags / YouTube push happen automatically).

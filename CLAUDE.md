...
- **Every sender must claim, and a stranded `'sending'` row is outcome-UNKNOWN** — `_claim_post_for_send` is the atomic `→ 'sending'` transition that stops two actors sending one row. `send_all_posts` was for a long time the only one of the four `poster.post` call sites that skipped it, and the worst one to skip: it snapshots approved rows before the batch and then sends from that stale list for minutes, so a DateTrigger firing meanwhile sent a row the loop then sent again. Separately, startup used to reset any `'sending'` row to `'approved'` "since nothing is in flight on a fresh process" — true, and irrelevant: the claim was taken before the crash and the platform may have accepted the request, with the only record of that being the `mark_posted` the shutdown prevented. `scheduled_at` survives, so the row fell into the overdue-recovery pass and went out twice; quitting the app mid-upload was enough. Such a row is now `mark_failed` with the ambiguity stated, because no guard downstream can settle it — `find_recent_duplicate_post` looks for a `'posted'`/`'sending'` row with the same content and excludes the row itself. `from_failed=True` widens the claim for **deliberate human sends only** (it is what makes the banner's Retry work at all, since every row there is `'failed'`); unattended senders must never take a failed row. And `mark_posted` must stay OUTSIDE the handler that marks a post failed — a DB error after a live post would otherwise show it as failed and invite a re-send nothing can catch.
- **Automatic retry is limited to connect-phase failures, and that is the whole design** — `services/send_failures.py` classifies a send exception at raise time (never by matching the stored message, which is written for a human and gets reworded). "Transient" is too coarse a test: a DNS failure, refused connection or pool timeout means the request **provably never arrived**, so re-sending cannot duplicate; a read/write timeout or a dropped connection means it **may have arrived and only the response was lost**, so a retry mints a second post in front of a real audience. `ThreadsPoster._PUBLISH_AMBIGUOUS_TRANSPORT_ERRORS` already drew that line for one call with a resolve loop behind it; `send_failures` generalises it for the automatic path, which has no such fallback. The cause walk follows `__cause__` **only** — `__context__` is merely whatever exception was in flight, not a causal claim, so an SDK re-authing inside a `ReadTimeout` handler and surfacing a `ConnectError` would otherwise read as safe — and ambiguity anywhere in the chain wins over a connect-phase link anywhere else. `ThreadsPublishOutcomeUnknown` is listed by name rather than left to luck, and `ENOENT` is deliberately absent from the connect-phase errnos (it is `FileNotFoundError`; a missing `ffprobe` would have been retried for a day as though the request never left). Anything unrecognised — auth, validation, the non-public-video refusal, a bare 5xx — defaults to **not** retryable, because a wrong "no" costs one click and a wrong "yes" cannot be undone. `retry_failed_sends_job` then has three defences: an atomic claim out of `'failed'` (the job and a user pressing Retry cannot both own the row), `find_recent_duplicate_post` (the one collision a row cannot see from itself — a smart queue re-adding a video creates NEW posts with the same content, so a stale failure retrying afterwards would be the second copy; it is retired to `'skipped'` rather than left looping), and `retry_until`, stamped once on the first failure so repeated failures cannot walk the deadline forward forever. Window comes from the queue's `missed_grace_hours` when there is a queue — "is this still worth posting late?" is editorial and the user answered it there.

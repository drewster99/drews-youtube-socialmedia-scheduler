...
- **Failed sends stay visible app-wide** — `GET /api/social/failed-posts` lists posts in status `failed`; `static/js/failed-sends-banner.js` (loaded by `base.html` on every page, same pattern as the reconcile banner) shows them until each post is retried successfully, dismissed, or deleted. `social_posts.status` is the single source of truth. An error that exists only in a transient toast is treated as unsurfaced: scheduled sends fail with no page open at all. Ordering is by `failed_at`, **never by `id`** — id is creation order, so a post written weeks ago that failed minutes ago sorted below one written yesterday that failed last week, which is how a five-day-old failure came to head the banner over four from the same afternoon. NULL `failed_at` is a pre-migration row, old by definition, and sorts last.
- **Dismissing a failed send hides the attempt, never the problem** — `social_posts.dismissed_at` (migration 050) is excluded by the banner query, and this deliberately reverses the earlier "no separate dismissed state" rule. That rule existed to stop a real failure being silenced and forgotten; the risk is answered by the write rule instead of by refusing the feature: **`mark_failed` clears `dismissed_at`**, so a retry that fails again un-dismisses the row and it returns to the banner with the new error. A recurring problem therefore cannot be permanently hidden. Deleting the post was already possible and is strictly worse — it destroys the error text and, for a scheduled post, the post itself. Only a `failed` post can be dismissed; any other status is a `409`, because on those the column would be dead weight and the request is a misunderstanding worth reporting.

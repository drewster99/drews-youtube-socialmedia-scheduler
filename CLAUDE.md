...
# CLI subcommands
youtube-publisher auth [client_secret.json]  # YouTube OAuth flow
youtube-publisher install                     # Install as background service (launchd/systemd)
youtube-publisher uninstall
...
- **`config.py`** — All configuration from env vars / `.env` file. Data lives in `~/.youtube-publisher/`
...
- **Credentials in Keychain** — On macOS, social media tokens stored in system Keychain (`com.youtube-publisher.*`), with encrypted JSON fallback on other platforms
...
| `YTP_DATA_DIR` | `~/.youtube-publisher` | Data directory |

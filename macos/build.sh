...
# arm64 only. The embedded interpreter is aarch64-apple-darwin (see
# PYTHON_STANDALONE_URL), so an x86_64 slice was never runnable: the app would
# launch on an Intel Mac and die as soon as it started the server. Building it
# cost time and bundle size for a slice that could not work.
swift build -c release --arch arm64 2>&1 | tail -5
SWIFT_BINARY=$(swift build -c release --arch arm64 --show-bin-path)/$SWIFT_TARGET
...
# resolves both at "$SITE_PACKAGES/_bin/". arm64 only, to match the app and the
# embedded interpreter; macOS 15 deployment target — the vision-lab CLI uses the
# new Vision API.
...
swiftc -O -target arm64-apple-macos15.0 "${CLIPCROP_SRCS[@]}" -o "$SITE_PACKAGES/_bin/clipcrop"
